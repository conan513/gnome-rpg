...
export class SystemMonitor {
    ...
    async _tick() {
        try {
            const [disk, net, uptimeMin] = await Promise.all([
                this._getDiskIO(),
                this._getNet(),
                this._getUptime(),
            ]);
            ...
            const metrics = {
                ...
                netTxKBs: net.tx,
                uptimeMin: uptimeMin,
                uptimeStr: this._formatUptime(uptimeMin),
            };
            if (this.onTick) this.onTick(metrics);
        } catch (e) {
            console.error('[SystemRPG] Tick error:', e.message);
        }
    }
    ...
    async _getDiskIO() {
        try {
            const file = Gio.File.new_for_path('/proc/diskstats');
