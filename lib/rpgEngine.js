...
export const XP_GUIDE = [
    { stat: 'STR', emoji: '⚔️', name: 'Strength', desc: 'Earned by high CPU usage. Run intensive tasks or compile code.' },
    { stat: 'INT', emoji: '🧠', name: 'Intelligence', desc: 'Earned by RAM usage. Keep many applications and tabs open.' },
    { stat: 'DEX', emoji: '🗡️', name: 'Dexterity', desc: 'Earned by Disk I/O (reads/writes) and mouse movement distance.' },
    { stat: 'CHA', emoji: '🌐', name: 'Charisma', desc: 'Earned by Network Traffic. Download or upload data.' },
    { stat: 'CON', emoji: '🛡️', name: 'Constitution', desc: 'Earned passively over time based on System Uptime.' },
    { stat: 'WIS', emoji: '🔮', name: 'Wisdom', desc: 'Earned by multitasking: changing focus, switching workspaces, clicking, scrolling, and copying text.' },
];
...
export class RpgEngine {
    ...
    getNextUnlockInfo() {
        ...
        if (overall < 10) {
            return { target: 10, current: overall, title: 'Dual-Class Unlock', desc: 'Reach Level 10 to combine 2 stats!', fraction: overall / 10.0 };
        } else if (overall < 20) {
            return { target: 20, current: overall, title: 'Triple-Class Unlock', desc: 'Reach Level 20 to combine 3 stats!', fraction: (overall - 10) / 10.0 };
        } else if (overall < 30) {
            return { target: 30, current: overall, title: 'Quad-Class Unlock', desc: 'Reach Level 30 to combine 4 stats!', fraction: (overall - 20) / 10.0 };
        } else if (overall < 40) {
            return { target: 40, current: overall, title: 'Penta-Class Unlock', desc: 'Reach Level 40 to combine 5 stats!', fraction: (overall - 30) / 10.0 };
        } else if (overall < 50) {
            return { target: 50, current: overall, title: 'True Ascendant', desc: 'Reach Level 50 to master all stats!', fraction: (overall - 40) / 10.0 };
        } else {
            return { target: 99, current: overall, title: 'Maximum Ascension', desc: 'You have unlocked all class combinations.', fraction: Math.min(1.0, (overall - 50) / 49.0) };
        }
    }
